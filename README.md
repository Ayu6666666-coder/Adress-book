...
项目现在支持部门ID管理系统，每个联系人都有对应的部门ID：

- **部门ID格式**：`dept_` + 哈希值（如：`dept_2d2s`）
- **自动生成**：系统会为新部门自动生成唯一ID
- **数据关联**：联系人通过部门ID与部门信息关联
...
```bash
# 为现有数据添加部门ID
node scripts/add-department-ids.js

# 调整联系人部门（示例）
node scripts/adjust-departments.js
```

### 当前部门列表

- 总经理室 (dept_2d2s): 5人
- 行政人事部（党办） (dept_6nr5): 10人  
- 计划采购部 (dept_3l3j): 7人
- 市场拓展部 (dept_2u6k): 9人
- 新能源事业一部 (dept_3ydm): 5人
- 新能源事业二部 (dept_3yhi): 6人
- 生产事业部 (dept_2q8j): 29人
- 安全保障部 (dept_30b6): 5人
- 临港事业部 (dept_2os0): 8人
- 其他 (dept_vnw): 3人
...
### 2024年12月 - 部门ID功能更新
- **新增部门ID字段**：为每个联系人添加了唯一的部门ID标识，便于数据管理和关联
- **部门管理功能**：添加了部门列表和详细信息管理，支持查看部门统计信息
- **数据结构优化**：增加了departmentList字段，包含部门ID、名称和描述信息
- **自动ID生成**：为现有数据自动生成部门ID，确保数据完整性
- **联系人详情增强**：在联系人详情页面显示部门ID信息
- **人员调整记录**：
  - 周军：从市场拓展部调整到安全保障部（第二位）
  - 乐天：从安全保障部调整到市场拓展部（最后一位）
  - 施森山：从生产事业部调整到安全保障部
